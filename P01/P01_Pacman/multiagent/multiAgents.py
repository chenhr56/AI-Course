    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        if ghostPositions:
            ...
            for ghostPos in ghostPositions:
                ghostList.append(util.manhattanDistance(newPos, ghostPos))
            cGhost = min(ghostList)
            if cGhost == 0:
                return -1145141919810
        else:
            return 1145141919810
        ...
        for foodPos in newFood.asList():
            FoodList.append(util.manhattanDistance(newPos, foodPos))

        cFood = min(FoodList)

        return successorGameState.getScore() + sum(newScaredTimes) + 1.0 / (cFood * cGhost)
